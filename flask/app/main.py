    ...
    if not hasattr(g, 'neo4j_driver'):
        ...
        while True:
            try:
                ...
                g.neo4j_driver = GraphDatabase.driver(uri, auth=("neo4j", "test"))
...
def bacon_number():
    ...
    with driver.session() as session:
        ...
        result = session.run(
            'MATCH p=shortestPath((bacon:Person {name:"Kevin Bacon"})-[*]-(meg:Person {name:$name})) RETURN p, length(p) as bacon_number', name = name)
